...
# Copilot Instructions — Chamber 19 (org-shared)

> **Org:** `chamber-19`
> **Scope:** Family-wide guidance loaded by Copilot on every repo in the chamber-19 org.

These instructions are inherited by every repo in the Chamber 19 tool family. Per-repo `copilot-instructions.md` files are loaded **alongside** this file (not instead of it) — they hold the repo-specific rules that build on top of these family-wide foundations.

If a rule below conflicts with a repo-specific rule, the repo-specific rule wins. Org-shared rules are the floor, not the ceiling.
...
- Repo-specific discoveries that aren't in this instructions file (e.g. a subtle bug in a particular module and the regression test that pins it)
- Version-pin contracts with shared libraries (e.g. "this consumer's v1.x line expects `desktop-toolkit` ^2.3.0+")
...
2. Start with the lowest-level dependency. If a change touches `desktop-toolkit` and a consumer app, ship the toolkit change first, tag it, then bump the consumer's pin
3. Make each repo's PR self-contained. A consumer PR shouldn't say "this works once you merge #42 in `desktop-toolkit`." It should either pin to a released version or be explicitly marked "blocked on X."
