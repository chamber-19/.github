...
For the complete Chamber 19 reference — approved-verb table, `ConvertFrom-YamlCompat` pattern, `PowerShellForGitHub` guidance, and the table-separator fix — read `docs/skills/POWERSHELL.md` in `chamber-19/.github` when accessible.

## Chamber 19 hard rules

These apply to every script in `chamber-19/.github/scripts/` and override the general guidelines below when they conflict:

- **MUST** use only approved PowerShell verbs. Run `Get-Verb` to verify. Common unapproved verbs in Chamber 19 scripts and their replacements: `Escape-` → `ConvertTo-`, `Replace-` → `Update-`
- **MUST** set `$ErrorActionPreference = "Stop"` at the top of every script
- **MUST** use `[switch]` for boolean flags — never `[bool]`
- **MUST** use `"| $(@(‘---’) * $n -join ‘ | ‘) |"` for Markdown table separators — not `|---|`
- **MUST** read `$env:GITHUB_TOKEN` for GitHub API calls — never hardcode tokens
- **NEVER** use `Read-Host` — all input comes via parameters
