...
| Date repeated on each bullet for the same day | Keep one `### YYYY-MM-DD` heading and remove duplicated per-bullet dates |
| Entry has BOTH a `### YYYY-MM-DD` heading AND a `[YYYY-MM-DD]` per-bullet tag | Remove the per-bullet tag — the heading is the authoritative date; per-bullet dates are the old format and must never appear alongside a date heading |
